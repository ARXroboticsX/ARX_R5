...
namespace arx::r5
{
    R5Controller::R5Controller() : Node("l5_pro_controller_node")
    {
        ...
        joint_state_subscriber_ = this->create_subscription<arm_control::msg::PosCmd>(
            "ARX_VR_L", 10, std::bind(&R5Controller::CmdCallback, this, std::placeholders::_1));
        // 定时器，用于发布关节信息

        timer_ = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&R5Controller::PubState, this));

        l5_pro_Interfaces_ptr_ = std::make_shared<InterfacesThread>("can0",0);
    }

    void R5Controller::CmdCallback(const arm_control::msg::PosCmd::SharedPtr msg)
    {
